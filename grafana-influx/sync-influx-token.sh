...
if ! docker ps | grep -q "grafana-influx_influxdb_1"; then
...
ACTIVE_TOKEN=$(docker exec grafana-influx_influxdb_1 influx auth list --hide-headers 2>/dev/null | grep "admin's Token" | awk '{print $4}' || true)
...
if [ -z "$ACTIVE_TOKEN" ]; then
    ...
    docker exec grafana-influx_influxdb_1 influx setup \
        --force \
        --username admin \

...
- **State**: 8 named volumes (10 with ClickHouse), two of which — PostgreSQL
  ...
  their majors by hand because a major bump over a live volume initializes
...
`:<version>` and checks both platforms are in it. `:latest` is moved to
that manifest by a `promote` job only after every supported host has
installed the release and started it; a release that fails there leaves
`:latest` where it was. The
...
tag of that name (never `latest`, no version check) to exercise it before
a release; delete the tag and the package versions afterwards. Which tags
are accepted, and what each publishes, is `packaging/release-version.sh`.

...
echo -e "     Password: run ./show-passwords.sh --show-secrets (or grep WEB_ADMIN_PASSWORD $ENV_FILE)"
echo ""
echo -e "  📈 SmokePing: http://localhost:$(grep SMOKEPING_PORT "$ENV_FILE" | cut -d= -f2 || echo 8081)"
echo -e "     No authentication required"
echo ""
echo -e "${CYAN}💡 Tips:${NC}"
echo -e "  - View URLs and status: ./show-passwords.sh"
echo -e "  - ...with the secret values: ./show-passwords.sh --show-secrets"

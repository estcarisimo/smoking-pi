...
* **Web Admin** – Database-first target management interface with bulk operations
...
- **Database-Driven Configuration**: Centralized target management with web interface controls
...
2. [Database Choice: InfluxDB vs ClickHouse](#2-database-choice-influxdb-vs-clickhouse)
3. [Architecture](#3-architecture)
4. [Professional Dashboards](#4-professional-dashboards)
5. [Web Administration](#5-web-administration)
...
- Bash shell (for setup.sh script)
...
# Option 1: Setup with InfluxDB (default)
./setup.sh

# Option 2: Setup with ClickHouse
./setup.sh --database clickhouse
```

That's it! The setup script automatically:
- ✅ Generates secure passwords for all services
- ✅ Configures your chosen time-series database
- ✅ Starts all containers in correct order
- ✅ Synchronizes InfluxDB tokens (if using InfluxDB)
- ✅ Displays access URLs and credentials

### What the Setup Does

1. **Password Generation**: Creates secure passwords for PostgreSQL, Grafana, Web Admin, and your chosen database
2. **Database Selection**: Configures either InfluxDB or ClickHouse based on your choice
3. **Service Orchestration**: Starts all services with proper dependencies
4. **Token Synchronization**: For InfluxDB, ensures Grafana can connect properly
5. **Health Verification**: Checks that all services are running correctly
...
## 2. Database Choice: InfluxDB vs ClickHouse

The Pro edition supports two time-series databases:

### InfluxDB (Default)
- **Pros**: 
  - Mature ecosystem with excellent Grafana integration
  - Built-in UI for data exploration
  - Efficient storage for time-series data
  - Token-based authentication
- **Best for**: Most users, proven stability
- **Setup**: `./setup.sh` or `./setup.sh --database influxdb`

### ClickHouse
- **Pros**:
  - Extremely fast queries on large datasets
  - SQL-based queries
  - Column-oriented storage
  - Better compression ratios
- **Best for**: Very large deployments, SQL familiarity
- **Setup**: `./setup.sh --database clickhouse`
...
- **Security**: Admin password and API token generated by `setup.sh`
...
| `INFLUX_TOKEN`  | API token with write perms      | Generated by `setup.sh` |
| `INFLUX_ORG`    | InfluxDB organization           | `smokingpi`                  |
...
| `POSTGRES_PASSWORD` | PostgreSQL database password | Generated by `setup.sh` |
| `RRD_DIR`       | Where SmokePing stores RRDs     | `/var/lib/smokeping`         |
| `TZ`            | Timezone for all services       | `UTC` (auto-detected)        |
| `SECRET_KEY`    | Flask web admin secret key      | Generated by `setup.sh` |
| `WEB_ADMIN_USERNAME` | Web admin interface username | `admin` |
| `WEB_ADMIN_PASSWORD` | Web admin interface password | Generated by `setup.sh` |
| `GF_SECURITY_ADMIN_USER` | Grafana admin username     | `admin` |
| `GF_SECURITY_ADMIN_PASSWORD` | Grafana admin password | Generated by `setup.sh` |
| `GF_SECURITY_SECRET_KEY` | Grafana cookie signing key | Generated by `setup.sh` |
| `DOCKER_INFLUXDB_INIT_PASSWORD` | InfluxDB admin password | Generated by `setup.sh` |

Set them in **`docker-compose.yml`** or in `.env`.

> **⚠️ Security Note:** You MUST run `./setup.sh` before deploying manually. The setup script automatically generates secure random passwords and creates the `.env` file. The `.env` file is gitignored to prevent accidental commits of secrets.

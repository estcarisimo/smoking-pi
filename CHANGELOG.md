...
- **The assistant could not see the HTTP and TCP targets, and a wrong
  ...
  targets on the reference Pi, up from 18). And a name that is not a target
  ...
  (`cloudflare`) and a pointer to `get_microcut_stats` for the CPE, instead
  of an empty result that reads like an outage.

...
def restart_smokeping():
    """Restart SmokePing via the config-manager API"""
    try:
        result = config_api.restart_smokeping_service()

        if str(result.get('success')).lower() == 'true':
            return jsonify({
                'success': True,
                'message': result.get('message', 'SmokePing restarted successfully')
            })
        else:
            return jsonify({
                'success': False,
                'error': result.get('message', 'Failed to restart SmokePing')
            }), 500
...
def get_oca_status():
    """Get current OCA status and last refresh time via config API"""
    try:
        if config_api.is_database_available():
            result = config_api.get_all_targets_from_db(
                active_only=True, category='netflix_oca'
            )
            oca_targets = result.get('targets', [])
            last_updated = max(
                (t.get('updated_at') or '' for t in oca_targets), default=''
            ) or 'Never'
        else:

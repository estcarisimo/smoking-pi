"""InfluxDB 2.x query helpers, shared by every module that reads the TSDB.

Reads the measurements the exporters write:
...
turns 100% into 1%, and the mistake is invisible until someone reads a
graph. See ``CLAMP_LOSS_RATIO``, which applies to the ratio series ONLY.

``DEFAULT_INFLUX_URL`` is loopback because the alerter runs with
``network_mode: host``; bridged services reach InfluxDB by service name. In
practice every compose service sets ``INFLUX_URL`` explicitly, so this
default only applies to tests and ad-hoc runs.
